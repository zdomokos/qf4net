...
{
    private RichTextBox   textBoxHistory;
    ...
    private void DispatchStartMessage()
    {
        ...
        var calcEvent = new RtcEvent(RtcSignals.Start);
        //RunToCompletion.Instance.Dispatch(calcEvent);
        var d  = new Dispatcher(RunToCompletion.Instance.Dispatch);
        var ar = d.BeginInvoke(calcEvent, Finished, d);
    }

    private void Finished(IAsyncResult iar)
    {
        var d = (Dispatcher)iar.AsyncState;
        d.EndInvoke(iar);
        UpdateForFinished();
    }
    ...
    private void UpdateForFinished()
    {
        ...
        if (InvokeRequired)
        {
            var finishDelegate = new FinishDelegate(UpdateForFinished);
            Invoke(finishDelegate);
        }
        else
        {
            if (textBoxState.Text != "Completed" && !Abort.Status)
            {
                DispatchStartMessage();
            }
            else
    ...
    private void UpdateRtcStatus(object sender, RtcDisplayEventArgs e)
    {
        ...
        if (InvokeRequired)
        {
            RunToCompletion.RtcDisplayHandler rtcDisplayHandler =
                new RunToCompletion.RtcDisplayHandler(UpdateRtcStatus);
    ...
    private MainForm()
    {
        ...
        RunToCompletion.Instance.DisplayState += new RunToCompletion.RtcDisplayHandler(
                                                                                       UpdateRtcStatus
                                                                                      );
    }
    ...
    private void InitializeComponent()
    {
        ...
        this.textBoxHistory.Font = new System.Drawing.Font(
        ...
        // numericUpDown1
        //
        this.numericUpDown1.Increment   = new System.Decimal(new int[] { 10000000, 0, 0, 0 });
        this.numericUpDown1.Location    = new System.Drawing.Point(232, 18);
        this.numericUpDown1.Maximum     = new System.Decimal(new int[] { 1410065408, 2, 0, 0 });
        ...
        this.numericUpDown1.Value       = new System.Decimal(new int[] { 100000000, 0, 0, 0 });
        this.numericUpDown1.ValueChanged += new System.EventHandler(
