...
namespace DiningPhilosophers
{
    ...
    public class Philosopher : QActive
    {
        ...
        private QState Thinking(IQEvent qEvent)
        {
            ...
            {
                LogMessage(
                    String.Format("Philosopher {0} is exiting thinking state.", m_PhilosopherId)
                );
                return null;
            }
        ...
        private QState Hungry(IQEvent qEvent)
        {
            if (qEvent.IsSignal(QSignals.Entry))
            {
                ...
                LogMessage(
                    String.Format("Philosopher {0} publishes Hungry event.", m_PhilosopherId)
                );
            ...
            if (qEvent.IsSignal(DPPSignal.Eat))
            {
                ...
                {
                    LogMessage(
                        String.Format("Philosopher {0} receives eat signal.", m_PhilosopherId)
                    );
            ...
            {
                LogMessage(
                    String.Format("Philosopher {0} is exiting hungry state.", m_PhilosopherId)
                );
                return null;
            }
            return this.TopState;
        ...
        private QState Eating(IQEvent qEvent)
        {
            ...
            {
                LogMessage(
                    String.Format("Philosopher {0} is exiting eating state.", m_PhilosopherId)
                );
